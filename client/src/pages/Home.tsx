...
// ── Placeholder tweet — replace with real post URL ────────────────────────────
const MAIN_TWEET_URL = 'https://x.com/minizenhq/status/PLACEHOLDER';

const TASKS = [
  {
    key: 'follow' as TaskKey,
    label: 'Follow @minizenhq',
    url: 'https://x.com/minizenhq?s=21',
  },
  {
...
// ── Collection preview (5xx range) ───────────────────────────────────────────
const COLLECTION_PREVIEW = [
  '515.JPG','516.JPG','517.JPG','518.JPG',
  '519.JPG','520.JPG','521.JPG',
];

// ── Countdown (72h from session start) ───────────────────────────────────────
...
// ── FAQ ───────────────────────────────────────────────────────────────────────
const FAQS = [
  { q: 'What is Minizen?', a: 'Minizen is a 10,000 piece hand-drawn NFT collection. Old school cartoon energy, new school onchain.' },
  { q: 'How many?', a: '10,000 total supply. Each one unique. A handful of 1/1 honoraries gifted to community members.' },
  { q: 'How do I get on the allowlist?', a: 'Follow @minizenhq, retweet and quote tweet the main post, then submit your wallet below. Simple.' },
  { q: 'Is the mint free?', a: 'Allowlist details TBA. Follow @minizenhq for updates.' },
  { q: 'What chain?', a: 'TBA. Follow @minizenhq for the announcement.' },
  { q: 'When is the mint?', a: 'Soon. Stay locked.' },
];
  ...
  return (
    <section style={{ width: '100%', maxWidth: 640, padding: '4rem 0' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
        {FAQS.map((faq, i) => (
          <div key={i} style={{ borderTop: `2px solid ${c.ink}`, borderBottom: i === FAQS.length - 1 ? `2px solid ${c.ink}` : 'none' }}>
            <button
            ...
            >
              <span style={{ fontFamily: "'Space Mono', monospace", fontWeight: 700, fontSize: '0.82rem', color: c.ink, letterSpacing: '0.02em' }}>
                {faq.q}
              </span>
              <motion.span
                animate={{ rotate: open === i ? 45 : 0 }}
                style={{ fontFamily: 'monospace', fontSize: '1.2rem', color: c.ink, flexShrink: 0, marginLeft: 12 }}
              >+</motion.span>
            </button>
            <AnimatePresence>
              {open === i && (
                ...
                >
                  <p style={{ fontFamily: "'Space Mono', monospace", fontSize: '0.78rem', color: c.inkLight, lineHeight: 1.8, paddingBottom: '1.1rem', margin: 0 }}>
                    {faq.a}
                  </p>
                </motion.div>
              )}
...
function ConfirmModal({ onYes, onNo }: { onYes: () => void; onNo: () => void }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      onClick={onNo}>
      <motion.div
        initial={{ y: 24, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 10, opacity: 0 }}
      ...
      >
        ...
        <h2 style={{ fontFamily: "'Bebas Neue', cursive", fontSize: '2rem', fontWeight: 400, color: c.ink, marginBottom: '0.5rem', letterSpacing: '0.06em' }}>
          You a Minizen?
        </h2>
        <p style={{ fontFamily: "'Space Mono', monospace", fontSize: '0.75rem', color: c.inkLight, lineHeight: 1.7, marginBottom: '2rem' }}>
...
function WhitelistModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      await submitApplication({
        ...
        xUsername: xLink.trim(),
        quoteTweet: inputs.quote?.trim() || xLink.trim(),
      });
      onSuccess();
    } catch {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      onClick={onClose}>
      <motion.div
        initial={{ y: 32, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 14, opacity: 0 }}
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.75rem' }}>
          <div>
            <h2 style={{ fontFamily: "'Bebas Neue', cursive", fontSize: '1.75rem', fontWeight: 400, color: c.ink, letterSpacing: '0.06em', lineHeight: 1 }}>
            ...
            return (
              <div key={task.key}>
                <div style={{
                  ...
                  <div style={{
                    width: 18, height: 18, border: `2px solid ${isDone ? c.white : c.ink}`,
                    background: isDone ? c.white : 'transparent',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                    transition: 'all 0.2s',
                  }}>
                    {isDone && <span style={{ fontSize: '0.6rem', color: c.ink, fontWeight: 900 }}>✓</span>}
                  </div>
                  <span style={{ flex: 1, fontFamily: "'Space Mono', monospace", fontSize: '0.75rem', color: isDone ? c.white : c.ink, fontWeight: 700, transition: 'color 0.2s' }}>
                    {task.label}
                  </span>
                  <button onClick={() => openTask(task)} style={{
                    background: isDone ? 'transparent' : c.ink,
                    border: `2px solid ${isDone ? 'rgba(255,255,255,0.3)' : c.ink}`,
                    color: isDone ? 'rgba(255,255,255,0.5)' : c.white,
                    fontFamily: "'Space Mono', monospace",
                    fontWeight: 700, fontSize: '0.65rem', padding: '4px 10px', cursor: 'pointer',
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      >
        <motion.div
          ...
          style={{ fontSize: '3rem', marginBottom: '1.25rem', lineHeight: 1 }}
        >◉</motion.div>
        <h2 style={{ fontFamily: "'Bebas Neue', cursive", fontSize: '2rem', fontWeight: 400, color: c.ink, letterSpacing: '0.06em', marginBottom: '0.5rem' }}>
          You're In The Queue
        </h2>
        <p style={{ fontFamily: "'Space Mono', monospace", fontSize: '0.75rem', color: c.inkLight, lineHeight: 1.8, marginBottom: '1.75rem' }}>
          Application received. We review manually. Turn on notifs @minizenhq.
        </p>
        <button onClick={onClose} style={{ ...inkBtn, width: '100%', padding: '0.9rem' }}>
          Close
        </button>
      </motion.div>
    </motion.div>
...
export default function Home() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: c.bg, color: c.ink, fontFamily: "'Space Mono', monospace" }}>
      ...
      <nav style={{
        ...
        <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <img src="/one/(1)logo.png" alt="Minizen" style={{ height: 32, width: 32, objectFit: 'contain' }}
              onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }}
            />
            <span style={{ fontFamily: "'Bebas Neue', cursive", fontSize: '1.3rem', letterSpacing: '0.12em', color: c.ink }}>
              MINIZEN HQ
            </span>
          </div>
          <a
            href="https://x.com/minizenhq?s=21"
            target="_blank"
            rel="noopener noreferrer"
            style={{ display: 'flex', alignItems: 'center', gap: 6, color: c.inkLight, fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', textDecoration: 'none', textTransform: 'uppercase' }}
          >
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '2rem', flexWrap: 'wrap' }}>
            <div style={{ flex: '1 1 280px' }}>
              ...
              <motion.p
                ...
                style={{ fontSize: '0.8rem', color: c.inkLight, lineHeight: 1.9, marginBottom: '2rem', maxWidth: 320 }}
              >
              ...
              <motion.button
                ...
                style={{
                  ...
                  fontSize: '0.82rem',
                  padding: '1rem 2.5rem',
                  boxShadow: `4px 4px 0 ${c.inkLight}`,
                  transition: 'box-shadow 0.15s',
                }}
              >
                Apply for Allowlist →
            ...
            >
              <img
                ...
                onError={e => {
                  (e.target as HTMLImageElement).src = '/517.JPG';
                }}
              />
            </motion.div>
          </div>
      ...
      {/* Footer */}
      <div style={{ borderTop: `2px solid ${c.ink}`, marginTop: '2rem' }}>
        <div style={{ maxWidth: 680, margin: '0 auto', padding: '2rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <span style={{ fontFamily: "'Bebas Neue', cursive", fontSize: '1.1rem', letterSpacing: '0.15em', color: c.inkLight }}>
            MINIZEN HQ
          </span>
          <span style={{ fontSize: '0.6rem', color: c.inkFaint, letterSpacing: '0.12em', textTransform: 'uppercase' }}>
            © 2025 · Draw your own path.
          </span>
          <a href="https://x.com/minizenhq?s=21" target="_blank" rel="noopener noreferrer"
            style={{ display: 'flex', alignItems: 'center', gap: 5, color: c.inkFaint, fontSize: '0.65rem', fontWeight: 700, textDecoration: 'none', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
